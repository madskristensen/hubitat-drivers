...
> **Status: v0.1.11 — beta. Hardware-tested LAN control of the Touchstone Sideline Elite. Generalizable to other Tuya WiFi fireplace models via Device Profile selection and in-driver DP discovery.**
>
> **Latest: v0.1.11** — removed `setLogBrightness` command (DP 105 confirmed read-only on Sideline Elite firmware); added defensive input-validation to enum setters.
>
...
- **`defaultFlameColor`** — Applied ~1.5s after device turns on (when using Sideline Elite profile)
- **`defaultFlameBrightness`** — Applied ~1.5s after device turns on (when using Sideline Elite profile)
- **`defaultLogColor`** — Applied ~1.5s after device turns on (when using Sideline Elite profile)
