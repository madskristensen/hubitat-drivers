...
> **Status: v0.1.1 — beta. Hardware-tested LAN control of the Touchstone Sideline Elite. Generalizable to other Tuya WiFi fireplace models via Device Profile selection and in-driver DP discovery.**
>
> **Latest: v0.1.1** — Adds Device Profile switching (Sideline Elite, Generic Tuya, Custom), in-driver DP discovery commands (`discoverDPs()`, `captureBaseline()`, `captureDiff()`), custom DP mapping, and auditable raw DP writes (`setRawDP()`).
>
...
- **`power`** — Device power state (`on` / `off`)
...
**Root cause:** You're on Hubitat driver v0.1.0 or v0.1.1. CRC32 import is required for Tuya packet validation.

**Fix:** Ensure you're running **v0.1.1 or later**. If you see this error, you have an old version. Delete the driver and re-import from GitHub.
