...
> **Status: v0.1.20 — beta. Hardware-tested LAN control of the Touchstone Sideline Elite. Generalizable to other Tuya WiFi fireplace models via Device Profile selection and in-driver DP discovery.**
>
> **Latest: v0.1.20** — **Active-TCP IP discovery + improved DHCP error message.** Added `discover` command button that scans the local /24 subnet on port 6668 to locate the fireplace when its IP changes after a DHCP lease renewal. The driver tries the ±20 IP range around the last known address first (typically finds the device in <1 min), then does a full /24 sweep. A new `networkAddress` attribute surfaces the discovered IP. Connection failures now log as `log.error` with actionable guidance (set a DHCP reservation, or press Discover). **Recommended first step: set a DHCP reservation in your router** — zero driver code, prevents IP drift entirely.
>
...
#### Charcoal color palette (DP 104 — verified Tuya app labels)

| DP value | Label       | Notes |
|----------|-------------|-------|
| `"1"`    | Orange      | Default selected in Tuya app |
| `"2"`    | Red         | |
| `"3"`    | Blue        | |
| `"4"`    | Yellow      | |
| `"5"`    | Green       | |
...
| `"12"`   | Spotlight   | ⚠️ Best-guess label — visually a mostly-white circle with a small orange wedge in the Tuya app. Rename in a future version once the actual Tuya label is confirmed. |
...
Both palettes have been verified from the Tuya app:

**Flame color (DP 101, 6 values):** Orange / Blue / White / Orange+Blue / Orange+White / Blue+White

**Charcoal color (DP 104, 12 values):** Orange / Red / Blue / Yellow / Green / Purple / Cyan / Magenta / White / Pink / Rainbow / Spotlight

> "Spotlight" (DP 104 value 12) is a best-guess label — visually a mostly-white circle with a small orange wedge in the Tuya app. It will be renamed in a future version if the actual Tuya label is confirmed.

## Breaking Changes

### ⚠️ Breaking change in v0.1.17 — `setLogColor` renamed to `setCharcoalColor`

The `setLogColor` command, `logColor` attribute, and `defaultLogColor` preference have been renamed to `setCharcoalColor`, `charcoalColor`, and `defaultCharcoalColor` to match the Tuya app's terminology (the Tuya app calls this control the "Charcoal" color picker, not "Log").

**Migration for Rule Machine automations:**
- Replace any `setLogColor(N)` action with `setCharcoalColor("LabelName")` using the verified label (e.g., `setCharcoalColor("Orange")` for palette position 1)
- There is no backward-compat alias for the old command name
- The DP number (104) is unchanged — only the command/attribute names changed

**Migration for `defaultLogColor` preference:**
- The old NUMBER preference (1–12) is replaced with a named ENUM dropdown
- Existing saved numeric values will not match the new labels and will be silently skipped
- Re-pick your preferred default from the new dropdown and click Save Preferences

## Network Setup and DHCP Reservation

The Touchstone Sideline Elite communicates over your local network via the Tuya WiFi module. The driver connects to the device's IP address, which must not change while Hubitat is running.

### Recommended: Set a DHCP Reservation

> **Set a DHCP reservation (static IP) for the fireplace's MAC address in your router.** This is the most reliable solution — zero driver code required, prevents IP drift permanently.

To do this:
1. Log into your router admin interface (typically at `192.168.1.1` or `192.168.0.1`).
2. Look for **DHCP** → **DHCP Reservations** (or "Static DHCP", "Address Reservation", or "MAC Binding" — exact wording varies by router).
3. Find the fireplace in the connected-devices list (look for a device named after the Tuya module or with an unknown manufacturer).
4. Assign it a fixed IP address and save.

Once reserved, the fireplace will always get the same IP even after reboots or power cycles.

### Fallback: Discover Command

If the IP did change (before you set up a reservation), use the `discover` command button on the device page:

1. Press **`discover()`** on the Hubitat device page.
2. The driver scans the local /24 subnet on TCP port 6668.
3. It tries the ±20 range around the last-known IP first (typical DHCP drift) — usually finds the device in under a minute.
4. If not found in the smart range, it does a full 1–254 sweep (up to ~8 minutes worst case).
5. On match (device ID confirmed), the **Device IP** preference is updated automatically and the driver reconnects.

> **Note:** The `discover` command fails-closed: it only accepts a match if the responding device's Tuya device ID matches your stored Device ID. Other Tuya devices on the network are ignored.
...
| **Charcoal Color DP** | number | 104 | DP number for charcoal/log color |
...
- **`defaultCharcoalColor`** — Applied ~1.5s after device turns on (when using Sideline Elite profile); select from named palette labels (Orange / Red / Blue / Yellow / Green / Purple / Cyan / Magenta / White / Pink / Rainbow / Spotlight)
...
8. **Test your commands**:
   - Use `setFlameColor(1)`, `setFlameBrightness("Brightest")`, etc. to confirm they work
