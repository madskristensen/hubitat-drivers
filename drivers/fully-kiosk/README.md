...
**v0.5.0** removes MQTT support after broker compatibility testing failed across multiple iterations. The supported path is the original local REST polling architecture: REST commands plus `/getDeviceInfo` polling on the configured interval.
...
| Version | Date       | Notes |
|---------|------------|-------|
| 0.5.0   | 2026-05-18 | Removed MQTT support after broker compatibility testing showed the handshake path was not reliable in Hubitat. The supported architecture is local REST polling only: REST commands plus `/getDeviceInfo` polling on the configured interval. |
